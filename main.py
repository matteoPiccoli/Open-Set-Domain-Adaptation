...
def get_args():
    ...
    parser.add_argument("--target", default='RealWorld', help="Target name")
    ...
    # Dataset path
    parser.add_argument("--path_dataset", default="drive/My Drive/Colab Notebooks/data/", help="Path where the Office-Home dataset is located")
    ...
    parser.add_argument("--epochs_step1", type=int, default=50, help="Number of epochs of step1 for known/unknown separation")
    parser.add_argument("--epochs_step2", type=int, default=50, help="Number of epochs of step2 for source-target adaptation")

    parser.add_argument("--train_all", type=bool, default=True, help="If true, all network weights will be trained")

    # Weights
    parser.add_argument("--weight_RotTask_step1", type=float, default=0.9, help="Weight for the rotation loss in step1")
    parser.add_argument("--weight_RotTask_step2", type=float, default=0.2, help="Weight for the rotation loss in step2")
    parser.add_argument("--weight_center_loss", type=float, default=0.001, help="Weight for the center loss in step1")
    parser.add_argument("--threshold", type=float, default=0.15, help="Threshold for the known/unkown separation")

    # Tensorboard logger
    parser.add_argument("--tf_logger", type=bool, default=True, help="If true will save tensorboard compatible logs")
    parser.add_argument("--folder_name", default=None, help="Used by the logger to save logs")

    # Multi-Head Rotation Classifier
    parser.add_argument("--is_multi", type=bool, default=True, help="If true will consider the multi-head rotation classifier")
...
class Trainer:
    def __init__(self, args):
        ...
        # Initialize the network with a number of classes equals to the number
        # of known classes + 1 (the unknown class, trained only in step2)
        self.feature_extractor = resnet18_feat_extractor()
        self.obj_classifier = Classifier(512, self.args.n_classes_known + 1)
        self.rot_classifier = Classifier(512*2, 4 *(1 + self.args.is_multi * (self.args.n_classes_known - 1)))
        
        self.feature_extractor = self.feature_extractor.to(self.device)
        self.obj_cls = self.obj_classifier.to(self.device)
        self.rot_cls = self.rot_classifier.to(self.device)

        source_path_file = 'drive/My Drive/Colab Notebooks/txt_list/' + args.source + '_known.txt'
        self.source_loader = data_helper.get_train_dataloader(args, source_path_file)

        target_path_file = 'drive/My Drive/Colab Notebooks/txt_list/' + args.target + '.txt'
    ...
    def do_training(self):
        ...
        # Restoring the std rotational classifier
        self.args.is_multi = False
        self.rot_classifier = Classifier(512*2, 4)
        self.rot_cls = self.rot_classifier.to(self.device)
        
        # New dataloaders
        source_path_file = 'drive/My Drive/Colab Notebooks/new_txt_list/' + self.args.source + '_known_' + str(rand) + '.txt'
        self.source_loader = data_helper.get_train_dataloader(self.args, source_path_file)

        target_path_file = 'drive/My Drive/Colab Notebooks/new_txt_list/' + self.args.target + '_known_' + str(rand) + '.txt'
        target_path_file_eval = 'drive/My Drive/Colab Notebooks/txt_list/' + self.args.target + '.txt'
